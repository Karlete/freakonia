...
Last updated: 2026-06-02 (sesión 74 — batch 119–123 + fixes freakochapas + callout PS1 en gaming)
...
- `js/articles-index.js` — array `ARTICLES_INDEX` con metadatos de los **135 artículos** (sin el campo `full`). Un objeto por línea. **Añadir nuevos artículos al TOP del array.**
...
```json
{"used_topics":["got","esports","gaming","magic","gamedev","warhammer","rol","nba"],"last_update":"2026-06-02"}
```
...
| 122 | `noticia-mtg-final-fantasy-prerelease-junio-2026` | magic | 02/06/2026 |
| 121 | `noticia-gamedev-summer-game-fest-week-2026-playstation-xbox` | gamedev | 02/06/2026 |
| 120 | `noticia-nba-finales-2026-spurs-knicks-1999-reencuentro` | nba | 02/06/2026 |
...
**Sesión 61 (2026-04-23):** Hover effects en gaming history widget — `index.html` y `gaming.html`: `.gaming-history-card:hover { box-shadow: var(--glow-blue); }` + transición `0.2s`. CSS ya tenía `border: 4px solid var(--accent-tertiary)` (azul eléctrico) como borde base del widget.
**Sesión 60 (2026-04-23):** Quiz redirect pages — 10 nuevas páginas HTML creadas (5 boy + 5 girl) con OG + Twitter/X meta tags específicos por arquetipo y `window.location.replace()` para redirigir a `quiz.html?resultado=X&gender=Y`. `quiz.html`: `shareOnX()` actualizado — `var url = window.location.href` reemplazado por objeto `shareUrls[gender][key]` con URLs absolutas por arquetipo; fallback a `https://freakonia.com/quiz.html`. `css/pixels.css`: añadido z-index:2 a `#que-se-cuece` y `#gaming-history-widget` para que queden sobre el canvas de lluvia (z-index:1).
**Sesión 59 (2026-04-23):** Gaming history widget — `js/gaming-history.js` creado: objeto `GAMING_HISTORY` con eventos por fecha `DD-MM`, ventana de 7 días desde hoy. Widget `#gaming-history-widget` añadido en `index.html` (entre `#que-se-cuece` y `#pixel-patron`) y en `gaming.html` (justo antes de `#canal`/`<hr>`). CSS `.gaming-history-*` inline en `<style>` de ambas páginas (border azul, VT323, sin border-radius). IIFE de renderizado inline antes de `</body>` en ambas páginas. `gaming.html`: bloque `<!-- Placeholder — más juegos -->` del MS2 sustituido por callout Freakochapa con imagen `ghoulsandghosts.webp`; imágenes `ghoulsandghosts.webp` y `asterix.webp` en flex row.
